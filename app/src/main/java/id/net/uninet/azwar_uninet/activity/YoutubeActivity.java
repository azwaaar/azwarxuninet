...
public class YoutubeActivity extends AppCompatActivity {
    ...
    @BindView(R.id.youtubeMovies)
    YouTubePlayerView youtubeMovies;
    Movie movie;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        movie = (Movie) getIntent().getSerializableExtra("serial_yt");

        getMoviesVideo();

    }

    private void getMoviesVideo() {
        StringRequest request = new StringRequest(Request.Method.GET, MainConfig.VIDEOS_MOVIE + movie.getId() + "/videos",
                response -> {
                    try {
                        ...
                        if (videoArrayList.isEmpty()) {
                            Log.d("TAG", "getMoviesVideo: is empty");
                            FancyToast.makeText(YoutubeActivity.this, "Maaf, Konten Tidak Tersedia Pada API themoviedb", FancyToast.LENGTH_LONG, FancyToast.INFO, false).show();
                        } else {
                            actionBar.setTitle(videoArrayList.get(0).getSite());

                            getLifecycle().addObserver(youtubeMovies);
                            youtubeMovies.addYouTubePlayerListener(new AbstractYouTubePlayerListener() {
                                @Override
